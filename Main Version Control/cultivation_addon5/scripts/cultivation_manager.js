...
const CONFIG = {
  DEBUG: true,
  MODULE_INIT_TIMEOUT: 30000,
  MODULE_POLL_INTERVAL: 100,
  MEDITATION_CHECK_INTERVAL: 1,
  ROGUE_CULTIVATOR_SPAWN_INTERVAL: 300,
  AUTOSAVE_INTERVAL: 500,
  ENLIGHTENMENT_CHANCE: 0.15,
  STAGES: {
...
function debugLog(message) {
  if (CONFIG.DEBUG) {
      ...
      console.warn(`[DEBUG ${timestamp}] ${message}`);
      try {
...
function checkEnlightenment() {
  return Math.random() < CONFIG.ENLIGHTENMENT_CHANCE;
}
...
async function initializePlayer(player, savedPower = 0) {
  try {
      ...
      debugLog(`Failed to initialize player ${player.name}: ${error.toString()}`);
  }
}
...
function applyCombatModifiers(player, stage) {
  try {
      const stageInfo = CONFIG.STAGES[stage];
      if (!stageInfo) {
...
}, CONFIG.MEDITATION_CHECK_INTERVAL);
...
}, CONFIG.DYNAMIC_PROPERTY_CHECK_INTERVAL);
...
}, CONFIG.ROGUE_CULTIVATOR_SPAWN_INTERVAL);
