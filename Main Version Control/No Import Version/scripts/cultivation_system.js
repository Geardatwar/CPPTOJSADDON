...
class CultivationSystem {
  constructor(savedData = null) {
      ...
      this.isAlive = true;  // Add this line
  }
...
world.afterEvents.playerSpawn.subscribe((eventData) => {
    ...
    if (!cultivationSystems.has(player.id)) {
        ...
        player.sendMessage("§e- Release shift to continue");
...
function applyCombatModifiers(player, stage) {
    try {
        if (stage.damageBonus > 0) {
            player.runCommand(`effect @s strength ${stage.damageBonus} 1 true`);
        }
        if (stage.defenseBonus > 0) {
            player.runCommand(`effect @s resistance ${stage.defenseBonus} 1 true`);
        }
    } catch (e) {
